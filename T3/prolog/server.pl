...
%initboard
parse_input(initBoard, Board) :- board_start(Board).

%checkend
parse_input(checkEnd(Play,Board), [NewPlay,NewEnd]) :- checkEnd(Play, NewPlay,Board,NewEnd).

%askplay normal player
parse_input(askPlay(Player,Row,Col,NewRow,NewCol,Board), NewBoard3) :-
...
checkEatMore(Player,NewBoard2,NewRow,NewCol,1,NewBoard3).

%askplaybot
parse_input(askPlayPC(Play,Board), NewBoard) :-
play(Play, Board, NewBoard, 3).
