...
MyBezierAnimation.prototype.getTotalDistance = function() {
  ...
  var totalDistance =
  this.calculateDistance(this.p1, l2) +
  this.calculateDistance(l2, l3) +
  this.calculateDistance(l3, r2) +
  this.calculateDistance(r2, r3) +
...
MyBezierAnimation.prototype.bezierFormula = function(time){
  ...
  if(this.t < 1){
    ...
    this.angle = Math.atan(dx / dz) + (dz < 0 ? 180.0*(Math.PI / 180) : 0);
    ...
    currPoint.push(this.x, this.y, this.z);

    this.t += 1.0/(this.calculateDistance(currPoint, this.p4 * 100));
